...
static int maxio_self_check(struct phy_device *phydev, int checknum)
{
	...
	for (i = 0; i < checknum; i++) {
		...
		if (0 == ret) {
			phydev_dbg(phydev, "MAE0621A READY\n");
			break;
		} else {
...
static int maxio_mae0621a_status(struct phy_device *phydev)
{
	...
	physr = maxio_read_paged(phydev, 0xa43, MAXIO_PHYSR_P_A43);
	if (physr < 0)
		return physr;

	if (physr & MAXIO_PHY_LINK) {
		phydev->link = 1;

		if ((physr & MAXIO_PHY_SPEED) == MAXIO_PHY_1000M)
			phydev->speed = SPEED_1000;
		else if ((physr & MAXIO_PHY_SPEED) == MAXIO_PHY_100M)
			phydev->speed = SPEED_100;
		else
			phydev->speed = SPEED_1000;

		if (physr & MAXIO_PHY_DUPLEX)
			phydev->duplex = DUPLEX_FULL;
		else
			phydev->duplex = DUPLEX_FULL;
...
{
	phydev_dbg(phydev, "maxio driver remove\n");
	if (phydev->priv != NULL) {
		kfree(phydev->priv);
	}
	phydev->priv=NULL;
...
static int maxio_mae0621a_config_init(struct phy_device *phydev)
{
	...
	phydev_dbg(phydev, "MAXIO_PHY_VER: %s\n", MAXIO_PHY_VER);
...
static int maxio_mae0621a_probe(struct phy_device *phydev)
{
	...
	phydev_dbg(phydev, "maxio_mae0621a_probe clkmode(oscillator) PHY_ID: 0x%x\n", phydev->phy_id);
...
static int maxio_mae0621aq3ci_probe(struct phy_device *phydev)
{
	...
	phydev_dbg(phydev, "maxio_mae0621aQ3C probe PHY_ID: 0x%x\n", phydev->phy_id);

	return 0;
}

static int maxio_mae0621aq3ci_config_leds(struct phy_device *phydev)
{
	int val;
	int ret;

	/* Green: link at any speed plus activity. */
	val = maxio_read_paged(phydev, 0xd04, 0x10);
	if (val < 0)
		return val;

	val |= BIT(5) | BIT(8) | BIT(10) | BIT(11);
	val &= ~BIT(14);
	ret = maxio_write_paged(phydev, 0xd04, 0x10, val);
	if (ret < 0)
		return ret;

	/* Yellow: steady link at any speed; disable EEE indication. */
	val = maxio_read_paged(phydev, 0xd04, 0x11);
	if (val < 0)
		return val;

	return maxio_write_paged(phydev, 0xd04, 0x11, val & ~BIT(3));
}
...
static int maxio_mae0621aq3ci_config_init(struct phy_device *phydev)
{
	...
	ret |= maxio_write_paged(phydev, 0xa42, 0x12, 0x28);
	ret |= maxio_write_paged(phydev, 0x0, 0x4, 0xde1);
	ret |= maxio_write_paged(phydev, 0x0, 0x9, 0x300); /* 1000Base-T Full/Half */
	ret |= maxio_write_paged(phydev, 0x0, 0x0, 0x9140);
	...
	ret = maxio_mae0621aq3ci_config_leds(phydev);
	if (ret < 0)
		return ret;
